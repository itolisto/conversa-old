...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SEL_MESSAGE_BY_ID`(IN `i_id` INT UNSIGNED) NOT DETERMINISTIC READS SQL DATA SQL SECURITY DEFINER
...
BEGIN
      SELECT
            ...
            `me_picture_thumb_file_id`AS `picture_thumb_file_id`,
